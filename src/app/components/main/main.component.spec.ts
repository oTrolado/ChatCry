import { async, ComponentFixture, TestBed } from '@angular/core/testing';
import { FormsModule } from '@angular/forms';
import { RouterTestingModule } from '@angular/router/testing';
import { HttpClientModule } from '@angular/common/http';
import { HttpClient } from '@angular/common/http';

import { TabsComponent } from '../shared/tabs/tabs.component';
import { TalkComponent } from '../shared/talk/talk.component';
import { TypeBarComponent } from '../shared/type-bar/type-bar.component';
import { MainComponent } from './main.component';
import { FilterContactsPipe } from '../shared/pipes/filter-contacts.pipe';
import { UrlPipePipe } from '../shared/pipes/url-pipe.pipe';
import { ChatsComponent } from '../shared/tabs/chats/chats.component';
import { GroupsComponent } from '../shared/tabs/groups/groups.component';
import { ContactsComponent } from '../shared/tabs/contacts/contacts.component';
import { DeleteDialogComponent } from '../shared/delete-dialog/delete-dialog.component';


describe('MainComponent', () => {
  let component: MainComponent;
  let fixture: ComponentFixture<MainComponent>;
  let contatoMock = {nome:'Jane Alinss', ultimoAcesso: new Date(), imagem:'base64/testesteste', mensagem:'o sol'}
  let contatoMock2 = {nome:'Jane Alberti', ultimoAcesso: new Date(), imagem:'base64/testesteste', mensagem:'o sol'}
  let talkMock = [{content: 'o sol', enter: false, self: false, url: "base64/testesteste"}];
  let talk2Mock = [{content: 'Perdi o pedal!', enter: false, self: false, url: "https://randomuser.me/api/portraits/women/53.jpg"}];
  let groupMock: any = {
    nome: "Cs na casa do mario",
    imagem: "http://cdn.vs.com.br/webedia-temp/1546522793354-csgo-duas-rambos.png",
    ultimaMensagem:{ 
        data: new Date(),
        mensagem: "Perdi o pedal!",
        usuario: "Aquele Carinha",
        imagem: "https://randomuser.me/api/portraits/women/53.jpg"
    }
  }

  beforeAll(() => {
    localStorage.setItem('chatCryProfile', '{"name":"Robso Basilio","password":"1","avatar":"data:image/jpeg;base64,","auth":true}');
  });

  beforeEach(async(() => {
    TestBed.configureTestingModule({
      imports: [ RouterTestingModule, HttpClientModule, FormsModule ],
      declarations: [ 
        MainComponent, 
        TabsComponent, 
        TalkComponent, 
        TypeBarComponent, 
        FilterContactsPipe, 
        ChatsComponent,
        UrlPipePipe,
        GroupsComponent,
        ContactsComponent,
        DeleteDialogComponent
      ],
      providers: [HttpClient]
    })
    .compileComponents();
  }));

  beforeEach(() => {
    fixture = TestBed.createComponent(MainComponent);
    component = fixture.componentInstance;
    fixture.detectChanges();
  });

  it('should create', () => {
    expect(component).toBeTruthy();
  });

  it('Teste de salvamento de chat na memoria antes da troca', () => {
    expect(component.startChat(contatoMock)).toEqual(talkMock);
    expect(component.startChat(contatoMock)).toBe('Same Name');
    
    const retorno = component.startChat(contatoMock2);
    expect(retorno).not.toBe('Same Name');
    expect(retorno).toEqual(talkMock);
  });

  it('Teste de inicio de chat com contato', () => {
    expect(component.chatWith(contatoMock)).toEqual(talkMock);
  });

  it('Teste de inicio de chat com grupo', () => {
    expect(component.chatWith(groupMock)).toEqual(talk2Mock);
  });

  it('Teste se a conversa será salva ao mandar nova mensagem', async () => {
    if(localStorage.getItem('chatCry'+contatoMock.nome))
      localStorage.removeItem('chatCry'+contatoMock.nome)
    component.lastTalk = contatoMock;
    component.sendMessage('Ola robso', true);

    await setTimeout(() => {
      expect(localStorage.getItem('chatCry'+contatoMock.nome)).toBeTruthy();
    }, 101);
  });
});
