import { async, ComponentFixture, TestBed } from '@angular/core/testing';
import { RouterTestingModule } from '@angular/router/testing';
import { TalkComponent } from '../shared/talk/talk.component';
import { TypeBarComponent } from '../shared/type-bar/type-bar.component';
import { LoginComponent } from './login.component';
import { resolve } from 'q';

describe('LoginComponent', () => {
  let component: LoginComponent;
  let fixture: ComponentFixture<LoginComponent>;
  const userMock = {
    name:"Robso Basilio",
    password:"1","avatar":
    "data:image/jpeg;base64,",
    "auth":true
  }
  beforeEach(async(() => {
    TestBed.configureTestingModule({
      imports: [ RouterTestingModule ],
      declarations: [ LoginComponent, TalkComponent, TypeBarComponent ]
    })
    .compileComponents();
  }));

  beforeEach(() => {
    localStorage.removeItem('chatCryProfile');
    fixture = TestBed.createComponent(LoginComponent);
    component = fixture.componentInstance;
    fixture.detectChanges();
  });

  it('deve criar', () => {
    expect(component).toBeTruthy();
  });

  it('Teste de inicialização', () => {
    component.ngOnInit()
      .then(resolve => expect(resolve).toBe('new user'))
      .catch(e => console.error(e));

    localStorage.setItem('chatCryProfile',JSON.stringify(userMock));

    component.ngOnInit()
      .then(resolve => expect(resolve).toBe('login'))
      .catch(e => console.error(e));

  });

  it('Teste de emissão de mensagem', () => {
    const mensagem = component.sendMessage('Ola teste', true);
    
    expect(typeof mensagem).toEqual('object');

    expect(mensagem['content']).toBe('Ola teste');

    expect(mensagem['self']).toBe(true);
  });

  it('Teste de mensagem emitida pelo type-bar', () => {
    const mensagem = component.receivedMessage('Teste jest');

    expect(mensagem).toBe('Teste jest');
  });

  it('Teste da validação de email', () => {
    const valido = component.validEmail('email@email.email');
    const invalido = component.validEmail('notavalidEmail@');

    expect(valido).toBe(true);
    expect(invalido).toBe(false);
  });

  it('Teste de escuta do nome do usuário', () => {
    component.getUserName();
    component.receivedMessage('Adonildo Garcio');

    expect(component.user['name']).toBe('Adonildo Garcio');

  });

  it('Teste de escuta do email do usuário', () => {
    component.getUserEmail();
    component.receivedMessage('useremail@emsd.schsa');
    
    expect(component.user['email']).toEqual('useremail@emsd.schsa'.toLowerCase);

  });

  it('Teste de escuta da senha do usuário', () => {
    component.getUserPassword();
    component.receivedMessage('1234');

    expect(component.user['password']).toBe('1234');

  });

  it('Teste de login', () => {
    component.user['password'] = '1234'
    component.getUserLogin();
    const resposta = component.receivedMessage('1234');

    expect(resposta).toBe('logando');

  });

  it('Teste de recomeço', () => {
    component.user = {name:'Jamal Anisio', email:'HUEHUEHUE', avatar:'base64/dfssfsfs', password:'1234'};
     const user = component.restart();

    expect(user).toEqual({name:null, email:null, avatar:null, password:null})

  });

  it('Teste de confirmação do cadastro', () => {
    const falha = component.confirmAvatar();
    expect(falha).toBe(false);

    component.user['avatar'] = 'base64/dsfsdfssdfsd';
    component.confirmAvatar()
    .then(res => {
      expect(res).toBe(true);
    });
  });

  it('Teste do botão de login', () => {
    const falha = component.loginButton();
    expect(falha).toBe(false);

    localStorage.setItem('chatCryProfile',JSON.stringify(userMock));

    const sucesso = component.loginButton();
    expect(sucesso).toBe(true);
  });

  it('Teste do "esquecer usuario"', () => {
    localStorage.setItem('chatCryProfile', JSON.stringify(userMock));
    const resposta = component.forget();
    expect(resposta).toBeNull();
  });
});
